SELECT *
  FROM (SELECT --T.COM, 
         ...
         T.TYPE,
         case t.PERIOD_NAME
           when '${periodName}' then
            'N'
           when cast(cast(substr('${periodName}', 1, 4) as int) - 1 as
                 varchar2(4)) || substr('${periodName}', 5) then
            'N-1'
           when cast(cast(substr('${periodName}', 1, 4) as int) - 2 as
                 varchar2(4)) || substr('${periodName}', 5) then
            'N-2'
           else
            NULL
          ...
          FROM (SELECT DECODE(R.CHANGE_SIGN, 'Y', -1, 1) *
                 ...
                 WHERE S.ROW_SET_ID = R.ROW_SET_ID
                   ...
                   AND IR.PERIOD_NAME in
                       ('${periodName}',
                        cast(cast(substr('${periodName}', 1, 4) as int) - 1 as
                             varchar2(4)) || substr('${periodName}', 5),
                        cast(cast(substr('${periodName}', 1, 4) as int) - 2 as
                             varchar2(4)) || substr('${periodName}', 5)) --参数期间
 ...
 ORDER BY T.TYPEORDER, T.ATTRIBUTE, T.PERIOD_NAME
