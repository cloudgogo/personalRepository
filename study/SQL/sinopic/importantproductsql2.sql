SELECT result.prov_code,
  ...
  FROM (SELECT map.prov_code,
          ...
          FROM (SELECT SUM(SsTotal) amount,
                 ...
                 ${if(isoilccard = 0,
                            ...
                            if(isoilccard = 1, 'and is_bind_oil_card=1', '')) }
                 ${if(len(mentype) = 0,
                            '',
                ...
                SELECT SUM(SsTotal) amount,
                       ...
                       commodity_code
                  FROM rep_nooil_transaction_mem_commodity_detail
                 WHERE XsDate BETWEEN date_format(date_add(STR_TO_DATE(concat('${startDate}',
                                                                              '-01'),
                                                                       '%Y-%m-%d'),
                                                           INTERVAL -
                                                           (TIMESTAMPDIFF(MONTH,
 ...
 WHERE cr. CODE = result.commodity_code
   AND cr.par_code = ci. CODE
   AND ci.lvl = ${if(len(producttype3)!=0,'3',if(len(producttype2)!=0,'2',if(len(producttype1)!=0,'1',null)))}
 GROUP BY result.prov_code, result.lvl_id, ci. CODE, ci. NAME
