USE [ESREPORT]
GO
/****** Object:  StoredProcedure [ODM].[Pro_DivideCost]    Script Date: 01/22/2018 21:34:48 ******/
SET ANSI_NULLS ON
GO
...
DECLARE public_expense_cursor CURSOR FOR 
   ...
   WHILE @@FETCH_STATUS = 0
       BEGIN
			       ...
			       WHILE @@FETCH_STATUS = 0
			           BEGIN
						   ...
						   ELSE 
				            ...
				            INSERT INTO ODM.COST_PUB_EXPENSE_TMP(OUT_ACCT_CODE,DEPT_ID,CUR_AMOUNT,IYPERIOD)
			                VALUES (@out_acct_code, 
				                    @dept_id, 
									@tailCountingAmount,
								    @iyperiod)
		...
		--合并分摊项元素 成为成本元素口径 插表
	    INSERT INTO ODM.COST_PUB_EXPENSE(COST_ELE_CODE, DEPT_ID, CUR_AMOUNT, IYPERIOD)
...
DECLARE @pub_sum_amount decimal(20, 2)
		...
		DECLARE full_consting_cursor CURSOR FOR 
		   ...
		   SELECT CT_CODE, CDEPT_ID,
		         case when  a.CDEPT_ID in ( select CDEPT_ID  --如果取数源是该部门的话，应该先从原金额中减去取数金额，再加上应该分摊的金额
	               ...
	               then (isnull(a.CUR_AMOUNT,0)- isnull(( select (CASE WHEN oaMap.O_ACCO_CODE = '7' THEN (SUM(fact.MD)*(2.5/7)) ELSE SUM(fact.MD) END) md
                                         from [ESREPORT].[ODM].[FCT_ACCASS] as fact, [ESREPORT].[ODM].[COST_OACCO_ACCO_MAP] as oaMap,esreport.ODM.COST_OACCO_COELE_MAP ocMap,
                                              [ESREPORT].[ODM].[COST_OACCO_DEPART_GET_MAP] as dpMap
                                         where fact.CCODE=oaMap.CCODE and oaMap.O_ACCO_CODE=dpMap.O_ACCO_CODE and fact.CDEPT_ID=dpMap.CDEPT_ID and 
                                               ocMap.O_ACCO_CODE=dpMap.O_ACCO_CODE and IYPERIOD=@max_iyperiod  and dpMap.CDEPT_ID=a.CDEPT_ID and ocMap.SRC_ELE_CODE=a.CT_CODE
                                         group by oaMap.O_ACCO_CODE, ocMap.SRC_ELE_CODE, dpMap.CDEPT_ID, fact.IYPERIOD
										),0)  --原金额-取数金额
						)
		   ...
		   WHERE a.IYPERIOD = @max_iyperiod and a.CT_CODE NOT IN (SELECT PARENT_CT_CODE FROM esreport.ODM.COST_ELEMENTS_INNER_MAPPING)
		   
		   UNION ALL
		   --取得分摊项表存在，但是成本分析表中不存在的数据
		   select a.COST_ELE_CODE, a.DEPT_ID, 0 CURRENT_AMOUNT, a.IYPERIOD, ISNULL(a.CUR_AMOUNT,0) pub_amount    --, b.CUR_AMOUNT or_amount, b.SUM_AMOUNT or_sumaount
	       from esreport.ODM.COST_PUB_EXPENSE a
	       where a.COST_ELE_CODE NOT IN (SELECT b.CT_CODE  FROM esreport.ODM.COST_CT_ANALYSIS b WHERE a.DEPT_ID=b.CDEPT_ID and a.COST_ELE_CODE=b.CT_CODE and a.IYPERIOD=b.IYPERIOD)
             ...
             BEGIN
			     IF(@pub_amount IS NULL)
				     BEGIN
					     SET @pub_amount = 0
				     END
			     INSERT INTO ODM.COST_DEPT_FULL_COSTING(CT_CODE,CDEPT_ID,CUR_AMOUNT,IYPERIOD)
			                VALUES (@ele_code, 
				                    @dept_id, 
...
--进行职能部门向业务部门的分摊
		...
		SELECT fullcost.CDEPT_ID, case when cast(s.CUR_AMOUNT as decimal(20,10))=0 then 0 else  cast(SUM(fullcost.CUR_AMOUNT) as decimal(20,10))/cast(s.CUR_AMOUNT as decimal(20,10)) end percentage INTO #busDeptPercen
			         ...
			         WHILE @@FETCH_STATUS = 0
			             BEGIN
						     ...
						     ELSE 
							  ...
							  INSERT INTO ODM.COST_FUNC_DEPT_FULL_COSTING(CT_CODE,CDEPT_ID,CUR_AMOUNT,SUM_AMOUNT,IYPERIOD)
			                      VALUES (@ele_code, 
				                          @dept_id, 
									      @tailCountingAmount,
...
--导入产品全成本使用的成本要素金额
    ...
    INSERT INTO ODM.COST_COST_DETAILS(DIST_CODE,CDEPT_ID,IYPERIOD,discurrentamount,discostvalue)
